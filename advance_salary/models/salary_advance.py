...
#def _employee_get(self):
    #print 'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBb ' + str(self.env.user.id)
    #raise osv.except_osv('', self._uid) 
    #ids = self.env['hr.employee'].search([('user_id','=',self.env.user.id)])
    #print ids

    #if ids:
    #    self.employee_id = ids[0]
    #return False
...
def _get_currency(self):
    print 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAaaaaa ' + str(self.env.user.id)
...
class SalaryAdvancePayment(models.Model):
    ...
    def _get_employee_id(self):
        #raise ValidationError(self.env.user.id)
        return self.env['hr.employee'].search([], limit=1)
        #return 1
    ...
    #    print 'AAAAAAAAAAAAAAAAAAAAAAAAAAa ' + str(self.employee_id)
    #    department_id = False
    #    if self.employee_id:
    #        employee = self.env['hr.employee'].search([('employee_id','=',self.employee_id)])
    #        department_id_id = employee.department_id.id

    #    return {'value': {'department': department_id}}

    @api.model
    def _employee_get(self):
        #return self.env.user.id if self.env.user.id else False
        raise ValidationError(self.env.user.id)
        print 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa' + str(self.env.user.id)
        return self.env['hr.employee'].search([], limit=1)[0]
    ...
    def compute_expense_totals(self, adv, company_currency, ref, account_move_lines):
